# Make OpenFret your own — in under 10 minutes

OpenFret is free and open source. There is nothing to buy, no account to make, and no monthly cost. Pick the path below that matches what you actually want.

## Pick your path

| If you want to... | Use this |
|---|---|
| Just use OpenFret and add your songs | **Path A: Use the live demo** (60 seconds) |
...
1. Open this URL on your phone or laptop: **https://awerb.github.io/openfret/**
...
To install it as an app on your phone: open the URL in Safari (iPhone) or Chrome (Android), tap the Share button, then **Add to Home Screen**. The "O" icon will appear like a normal app, and it opens full-screen without browser bars.

---

## Path B: Get your own free copy on the internet (recommended, ~10 minutes)

This gives you a permanent URL like `https://YOUR-USERNAME.github.io/openfret/`. You own the copy. It's free forever. No credit card. You can change it however you want.
...
"Forking" makes a copy of OpenFret under your own GitHub account.

1. Go to **https://github.com/awerb/openfret**.
2. Click the **Fork** button at the top right of the page.
3. On the next screen, leave everything as is and click **Create fork**.

You now have a copy at `https://github.com/YOUR-USERNAME/openfret`.
...
2. When it's ready, you'll see a green box near the top with your URL. It looks like `https://YOUR-USERNAME.github.io/openfret/`.
3. Click the **Visit site** button to confirm it works. You should see the OpenFret splash screen.

You now have your own copy of OpenFret, on your own URL, forever free.
...
|---|---|---|
| The site title (browser tab) | `index.html` | `<title>OpenFret</title>` |
| The splash screen image | `assets/openfret-banner.jpg` | Upload your own image with the same filename |
| The favicon (browser tab icon) | `assets/openfret-icon.png` | Upload your own square image |
| The app name when installed to a phone | `manifest.json` | `"name": "OpenFret"` |
...
3. Unzip the downloaded file. You'll get a folder called `openfret-main`.
...
**One caveat:** the microphone tuner needs your browser to consider the page "secure," and opening a local file doesn't always count. If the tuner doesn't work and you want it to, the easiest fix is to install Python (which most Macs already have) and from inside the openfret folder run `python3 -m http.server 8000`, then open `http://localhost:8000` in your browser.
...
You'll get a single JSON file named something like `openfret-library-2026-06-08.json`. Save it somewhere durable. Good choices:
...
1. Open OpenFret on the device where you want your songs.
2. Tap **LIBRARY** in the header.
...
OpenFret has no built-in sync (that would need a server, which would mean accounts, which would mean ads or fees). The export/import flow does the same job manually:
...
The JSON is human-readable. Open it in any text editor and you'll see your songs in plain English. This is on purpose: even if OpenFret disappeared tomorrow, your library would still be readable and convertible to anything else. No proprietary format, no lock-in.

---

## Sharing OpenFret with someone else

There are three ways to share:

1. **Send them the live demo URL** (https://awerb.github.io/openfret/) — works on any phone or computer, no signup, free.
...
OpenFret is MIT licensed, which means you can do almost anything with it: customize it, sell a customized version, give it away. All we ask is that you don't strip out the LICENSE file and that you don't sue us if something goes wrong.
...
**"I want to get help."** Open an issue at https://github.com/awerb/openfret/issues. Describe what you tried, what you saw, and what you expected.
